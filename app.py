...
APP_VERSION = "2025-08-16-home-with-register"
...
def ensure_cols(df: pd.DataFrame, required, name: str):
...
def build_proj_vectors_on_the_fly(projects: pd.DataFrame):
    ...
    for _, r in projects.iterrows():
        ...
        if pd.notna(r["region"]):        vec[f"region__{r['region']}"] = 1
        if pd.notna(r["sector_focus"]):  vec[f"sector__{r['sector_focus']}"] = 1
...
def status_dot_html(state):
    ...
    return (
        ...
        f'<span style="width:10px;height:10px;border-radius:50%;background:{color};display:inline-block;"></span>{label}'
        "</span>"
    )
...
def rule_score(donor_row, proj_row):
    ...
    try:
        ...
        if not np.isnan(budget_cap) and float(proj_row.get("funding_target", 1)) <= budget_cap:
            s += 0.15
    except Exception:
        pass
    s += (1.0 / (1.0 + float(proj_row.get("funding_target", 1)))) * 20000.0
...
def safe_df(df):
    if df is None or not isinstance(df, pd.DataFrame) or df.empty:
        return pd.DataFrame([{"Result": "N/A"}])
    return df
...
def load_core(base):
    ...
    if not interactions.empty:
        interactions = interactions.rename(columns={c: c.strip().lower() for c in interactions.columns})
        if not all(k in interactions.columns for k in ["donor_id", "project_id", "score"]):
            raise ValueError(f"Interactions must contain Donor_ID / Project_ID / Score (any casing). Found: {list(interactions.columns)}")
        ...
        interactions = interactions.rename(columns={
            "donor_id": "Donor_ID", "project_id": "Project_ID", "score": "Score"
...
# ───────────────────────────────────────────────────────────────────────────────
# SVD TRAIN (optional) + LOAD ARTIFACTS
# ───────────────────────────────────────────────────────────────────────────────
...
def load_artifacts(base):
    paths = {
        ...
        "don":  os.path.join(base, "donor_vectors.parquet"),
        "fe":   os.path.join(base, "feature_cols.json"),
    }
    missing = [p for p in paths.values() if not os.path.exists(p)]
    if missing: return None, None, None, None
    svd = joblib.load(paths["svd"])
    proj_vecs = load_csv_or_parquet(paths["proj"])
    donor_vecs = load_csv_or_parquet(paths["don"])
    feats = json.load(open(paths["fe"]))
...
def get_recs(
    ...
    # Why
    why = []
    rp = parse_multi(drow.get("region_preference"))
    sp = parse_multi(drow.get("sector_preference"))
    for _, r in cand.iterrows():
        parts = []
        if r["region"] in rp:        parts.append("Region match")
        if r["sector_focus"] in sp:  parts.append("Sector match")
...
# LOAD DATA + SESSION
# ───────────────────────────────────────────────────────────────────────────────
base_donors, projects, interactions = load_core(BASE)
svd_art, proj_vecs, donor_vecs, feats = load_artifacts(BASE)

if "donors" not in st.session_state:
    st.session_state["donors"] = base_donors.copy()
donors = st.session_state["donors"]

st.title("🤝 Diaspora Donor Recommender System")
st.caption("Hybrid (Rule • Cosine Similarity • CF via SVD). Home tab to find donor and set preferences, then insights, progress, metrics, explanations, explorer, comparisons, registration, and diagnostics.")
...
# ───────────────────────────────────────────────────────────────────────────────
# TABS (Home first, then all others)
# ───────────────────────────────────────────────────────────────────────────────
tab_home, tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8 = st.tabs([
    "🏠 Find donor and set preferences",
    "📊 Insights",
    "🧭 Donor progress",
    "📈 Metrics",
    "💡 Why these picks",
...
# ───────────────────────────────────────────────────────────────────────────────
# HOME (Find donor and set preferences + Recommendations)
# ───────────────────────────────────────────────────────────────────────────────
with tab_home:
    ...
    with left:
        ...
        if query:
            ...
            ddf = ddf[
                ddf.apply(
                    lambda r: q in str(r["donor_id"]).lower()
                    or q in str(r.get("name", "")).lower()
                    or q in str(r.get("email", "")).lower(),
                    axis=1,
                )
            ]
        hist_ids = set(interactions["Donor_ID"].unique()) if interactions is not None and not interactions.empty else set()
        ...
        cf_mode = st.radio("Collaborative filtering source", ["Use pre-trained (fast)", "Train inline from interactions"], index=0)
        ...
        if not has_rows(recs):
            ...
            with c2:
                ...
                if st.button("Save shortlist (PDF)"):
                    ...
                    st.success(f"Saved: {p}")
            with c3:
                st.download_button("Download current results (CSV)",
...
with tab1:
    ...
    if not has_rows(r):
        st.info("Generate recommendations in **Home**.")
    else:
...
with tab2:
    ...
    current_bt = drow.get("behaviour_type", "active")
    if pd.isna(current_bt) or str(current_bt).strip() == "": current_bt = "active"
    st.markdown("**Current status:** " + status_dot_html(current_bt), unsafe_allow_html=True)
    new_bt = st.selectbox("Behaviour type", ["active", "passive", "selective"],
                          index=["active","passive","selective"].index(current_bt))
    if new_bt != current_bt:
        donors.loc[donors["donor_id"] == donor_id, "behaviour_type"] = new_bt
        st.session_state["donors"] = donors
        st.success("Behaviour updated.")

    hist = interactions[interactions["Donor_ID"] == donor_id] if interactions is not None and not interactions.empty else pd.DataFrame()
...
with tab3:
    ...
    if not has_rows(r):
        ...
        hist_d = interactions[interactions["Donor_ID"] == donor_id] if (interactions is not None and not interactions.empty) else pd.DataFrame()
        donor_has_history = has_rows(hist_d)

        rel_thr = None
        if donor_has_history:
            scores = hist_d["Score"].astype(float)
            if    thr_mode == "Median per donor":          rel_thr = float(scores.median())
            elif  thr_mode == "60th percentile per donor": rel_thr = float(scores.quantile(0.60))
            else:                                          rel_thr = float(custom_thr if custom_thr is not None else 0.60)
        ...
        coveragek = 0.0
        if interactions is not None and not interactions.empty and len(topk_list) > 0:
            seen_items = set(interactions["Project_ID"].astype(str).unique())
            coveragek = sum(int(pid in seen_items) for pid in topk_list) / len(topk_list)
            ...
            for _, row_h in hist_d.iterrows():
                ...
                try:
                    ...
                    if pd.notna(pred) and pd.notna(row_h["Score"]):
                        y_true.append(float(row_h["Score"])); y_pred.append(float(pred))
                except Exception:
                    continue
            if len(y_true) >= 1:
                y_true = np.array(y_true, dtype=float); y_pred = np.array(y_pred, dtype=float)
                abs_err = np.abs(y_pred - y_true); sq_err = (y_pred - y_true) ** 2
        ...
        c4.metric("Coverage@K (CF)", f"{coveragek*100:.1f}%")
        c5.metric("Novelty ↑",        f"{novelty:.2f}")
        c6.metric("Diversity@K",      f"{diversity*100:.1f}%")

        st.markdown("**Error metrics on donor history (CF)**")
        ...
        labels = ["Rule", "Cosine", "CF (SVD)"]; vals = [means["rule_score_norm"], means["cosine_score_norm"], means["cf_score_norm"]]
        fig4, ax4 = plt.subplots(figsize=FIG_S)
        ax4.barh(labels, vals, color=[PALETTE[0], PALETTE[1], PALETTE[4]])
        ax4.set_xlim(0, 1); ax4.set_title("Average score components")
        for i, v in enumerate(vals):
            ax4.text(min(0.97, v + 0.02), i, f"{v:.2f}", va="center")
        st.pyplot(fig4)

        figp, axp = plt.subplots(figsize=FIG_XS)
        axp.scatter([precisionk], [recallk], s=40, color=PALETTE[2])
...
with tab5:
    ...
    cols_to_show = st.multiselect("Columns to display",
                                  options=list(projects.columns),
...
with tab6:
    ...
    def run_algo(weights, label):
        recs,_ = get_recs(
            donor_id, donors, projects, interactions, svd_art, None, None, None, weights,
    ...
    for w in [(1,0,0),(0,1,0),(0,0,1),(0.33,0.33,0.34)]:
        recs,_ = get_recs(
            donor_id, donors, projects, interactions, svd_art, None, None, None, w,
    ...
    axx.set_title("Average score by algorithm")
    st.pyplot(figx)

# ───────────────────────────────────────────────────────────────────────────────
# ➕ REGISTER DONOR (restored)
# ───────────────────────────────────────────────────────────────────────────────
with tab7:
    ...
    if submitted:
        ...
        st.success(f"Donor {new_id} created and pre-selected in Home. Generate recommendations there.")
...
with tab8:
    ...
    donors_ok = isinstance(donors, pd.DataFrame) and not donors.empty
    inter_ok = isinstance(interactions, pd.DataFrame) and not interactions.empty
    ...
    donors_with_hist_n   = len(donors_ids.intersection(hist_ids))
    projects_with_hist_n = len(proj_ids.intersection(hist_proj))
    ...
    if donors_ok and donors_with_hist_n > 0:
        sample_ids = list(donors_ids.intersection(hist_ids))[:10]
        ...
        st.info("No overlapping donor IDs found yet. Use Home to generate recommendations and Metrics to evaluate.")
